...
extern "C" {
	...
	int alg_yuv4202bgrplane(unsigned char* __restrict pYuvBuf, const int nWidth, const int nHeight, unsigned char* __restrict pRgbBuf)
	{
		...
		int8x8_t _v128 = vdup_n_s8((char)128);
	...
	int alg_yuv4202rgbplane(unsigned char* __restrict pYuvBuf, const int nWidth, const int nHeight, unsigned char* __restrict pRgbBuf)
	{
	#if defined(__ARM_NEON__) || defined(__ARM_NEON)
		if (NULL == pYuvBuf || NULL == pRgbBuf)
			{
			...
			unsigned char* r = pRgbBuf;
			unsigned char* g = r + (nYLen);
			unsigned char* b = r + ((nYLen) << 1);
		...
		unsigned char* r = pRgbBuf;
		unsigned char* g = r + (nWidth * nHeight);
		unsigned char* b = r + (nWidth * nHeight) * 2;
	...
	int alg_rgbcut2rgbplanemn(const unsigned char* __restrict pSrcBuf, const int nWidth, const int nHeight, float* __restrict pBgrBuf, const float* mean_vals, const float* norm_vals)
	{
		...
		const unsigned char* pSrcRgb = pSrcBuf;//��֯ģʽ
	...
	int alg_bgrcutCrop(unsigned char* pSrc, unsigned char* pDst, int startX, int startY, int imgSW, int imgSH, int imgDW, int imgDH) //bgr�����洢
	{
		...
		for (int h = 0; h < imgDH; h++)
		{
			...
			if (cur_y >= 0 && cur_y < imgSH)//y ������Դ��Χ��
			{
				for (int w = 0; w < imgDW; w++)
				{
					int cur_x = w + startX;
					if (cur_x >= 0 && cur_x < imgSW)// x ������Դ��Χ��
					{
	...
	float FastInvSqrtf(float x)
	{
		...
		float* Px = &x; //����gcc�� -Wstrict-alhusing ����
		size_t i = *(size_t*)Px;
		//size_t i = *(size_t*)&x;
...
#undef SUBSIZE
	...
	float algGetCosDistance(const float* v1, const float* v2, const unsigned int Size)
	{
		...
		sum = MAX(0.0f, sum);
		//sum = (sum / sqrt(mod1) / sqrt(mod2) + 1) / 2.0f;//cos���뷶Χ[-1,1]
